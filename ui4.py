...
class HugoEditor:
    ...
    def insert_code_block(self, e):
        """在光标位置插入代码块短代码"""
        # 获取当前光标位置
        selection = self.content_input.get_selection()
        if selection:
            start = selection.start
            end = selection.end
        else:
            # 如果没有选中文本，在光标位置插入
            start = self.content_input.cursor_position
            end = start

        # 获取当前文本
        current_text = self.content_input.value or ""
        
        # 要插入的代码块
        code_block = "{{< code >}}\n\n{{< /code >}}"
        
        # 在光标位置插入
        new_text = current_text[:start] + code_block + current_text[end:]
        
        # 更新文本
        self.content_input.value = new_text
        
        # 将光标移动到代码块中间（两个换行之间）
        new_cursor_pos = start + len("{{< code >}}\n")
        self.content_input.cursor_position = new_cursor_pos
        
        # 聚焦到内容输入框
        self.content_input.focus()
        
        self.status_text.value = "✅ 已插入代码块"
        self.status_text.color = ft.Colors.GREEN
        self.page.update()
        ...
        def clear():
            time.sleep(2)
            if main_loop and self.status_text.value == "✅ 已插入代码块":
    ...
    def setup_ui(self):
        ...
        # 工具栏按钮
        self.code_block_btn = ft.OutlinedButton(
            "📝 插入代码块", 
            icon=ft.Icons.CODE,
            on_click=self.insert_code_block,
            tooltip="在光标位置插入 {{< code >}} 短代码"
        )
